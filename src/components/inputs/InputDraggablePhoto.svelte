<script>
  ...
  const handleMousedown = (e) => {
    ...
    e.preventDefault();
    const containerRatio = containerRef.offsetWidth / containerRef.offsetHeight;
    direction = "Y";
    let reset = "X";
  ...
  const handleMousemove = (e) => {
    let difference = e[`page${direction}`] - startPos[direction];
...
<div class="photo" bind:this={containerRef} on:mousedown={handleMousedown}>
  {#if $cardStore.isRepositioning}
    <div>
      <button
        class="photo__repos--inner photo__repos--button"
        on:click={handleDoneClick}
      >
        Done
      </button>
      <div class="photo__repos--outer">
        <div class="photo__repos--inner">Drag to reposition</div>
      </div>
    </div>
  {/if}
  <img
